...
export default function GuestLayout({ children }) {
    return <div className="flex min-h-screen flex-col bg-bg-base">
        <header className="flex items-center justify-between gap-4 border-b border-border px-6 py-6 sm:px-12"><Link href={route('home')} className="flex items-center gap-3"><Brand/></Link><Link href={route('home')} className="text-sm text-text-secondary hover:text-lime-500">← Kembali ke beranda</Link></header>
        ...
        <footer className="border-t border-border px-6 py-6 text-center text-sm text-text-muted">Arus — simulasi perencanaan keuangan, bukan nasihat investasi.</footer>
    </div>;
}
